...
public class Lifecycle {
  ...
  public synchronized boolean tryToMoveTo(State to) {
    ...
    if (TRANSITIONS.get(currentState).contains(to)) {
      ...
      listeners.forEach(listener -> listener.failedTransition(currentState, to));
    }
  ...
  public interface LifecycleListener {
    /**
     * Called when a transition from state {@code from} to state {@code to} failed.
     */
    void failedTransition(State from, State to);
